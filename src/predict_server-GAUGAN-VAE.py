...
from SPADE.loadInference import infere_GAUGAN_VAE
...
class DonkeySimMsgHandler(IMesgHandler):
    ...
    def on_telemetryGAN(self, data):
        ...
        # Carga de la imagen de referencia de estilo
        styleImage = Image.open(self.style_image)
        image = styleImage.convert('RGB')

        transform = []
        transform.append(transforms.Lambda(lambda img: self.__scale_width_pil(img, 512)))
        ...
        styleImage = np.reshape(image_tensor, (1, 3, 256, 512))
        ...
        while(True):
            ...
            if len(imgNames) > 1:
                ...
                with torch.no_grad():
                    generada = infere_GAUGAN_VAE(self.modelDict['ganModel'], label, tamImagenes, styleImage)
    ...
    def __scale_width_pil(self, img, target_width, method=Image.BICUBIC):
...
def go(modelosName, address, constant_throttle=0, num_cars=1, image_cb=None, rand_seed=None, style_image = ""):
    ...
    if modelosName['GAN'] != None:
        os.chdir('./SPADE')
        ganModel = load_GAUGAN(modelosName['GAN'], use_vae = True)
    ...
    handler.style_image = style_image
    server = SimServer(address, handler)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--style_image', type=str, default= "", help="Imagen de estilo para GAUGAN.")
    ...
    go(modelosName, address, args.constant_throttle, num_cars=args.num_cars, rand_seed=args.rand_seed, style_image=args.style_image)
